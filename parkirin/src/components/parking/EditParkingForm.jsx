...
const EditParkingForm = ({ parking, onClose, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    address: '',
    location: {
      coordinates: [106.8456, -6.2088] // Default to Jakarta
    },
    capacity: {
      car: 10,
      motorcycle: 20
  ...
  // Initialize form with existing parking data
  useEffect(() => {
    if (parking) {
      setFormData({
        name: parking.name || '',
        ...
        location: {
          coordinates: parking.location?.coordinates || [106.8456, -6.2088]
        },
        capacity: {
  ...
  const validateForm = () => {
    ...
    // Note: location and capacity cannot be updated after creation
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);    try {
      ...
      setErrors({ submit: error.message || 'Gagal memperbarui parking lot' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-900">Edit Parking Lot</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          </div>          {/* Location Coordinates - Read Only */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Lokasi Parking Lot
            </label>
            <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-700">
                <span className="font-medium">Info:</span> Lokasi tidak dapat diubah setelah parking lot dibuat.
              </p>
            </div>
          ...
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-2">Kapasitas Parkir</h3>
            <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-700">
                <span className="font-medium">Info:</span> Kapasitas tidak dapat diubah setelah parking lot dibuat.
              </p>
            </div>
